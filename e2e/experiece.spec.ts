  ...
  for (const job of jobs) {
    test.describe(`${job.id} - Job Card Tests`, () => {
      test('should display job card and logo', async ({ page }) => {
        ...
        await expect(page.getByTestId(`job-website-link-${job.id}`)).toHaveAttribute(
          'href',
          job.website
        );
      });
        ...
        test.describe(`Role ${roleIndex}: ${role.title}`, () => {
          ...
          for (let paragraphIndex = 0; paragraphIndex < role.paragraphs.length; paragraphIndex++) {
            ...
            test(`should display paragraph ${paragraphIndex}`, async ({ page }) => {
              ...
              if (paragraph.title && paragraph.title.trim() !== '') {
                const paragraphTitle = page.getByTestId(
                  `job-paragraph-title-${job.id}-${roleIndex}-${paragraphIndex}`
                );
              ...
              const paragraphText = page.getByTestId(
                `job-paragraph-text-${job.id}-${roleIndex}-${paragraphIndex}`
              );
          ...
          if (role.achievements && role.achievements.list.length > 0) {
            test('should display achievements section', async ({ page }) => {
              ...
              await expect(
                page.getByTestId(`job-achievements-${job.id}-${roleIndex}`)
              ).toBeVisible();

              // Verify achievements title
              const achievementsTitle = page.getByTestId(
                `job-achievements-title-${job.id}-${roleIndex}`
              );
              ...
              await expect(
                page.getByTestId(`job-achievements-list-${job.id}-${roleIndex}`)
              ).toBeVisible();
            });

            for (
              let achievementIndex = 0;
              achievementIndex < role.achievements.list.length;
              achievementIndex++
            ) {
              const achievement = role.achievements.list[achievementIndex];

              test(`should display achievement ${achievementIndex}`, async ({ page }) => {
                const achievementItem = page.getByTestId(
                  `job-achievement-item-${job.id}-${roleIndex}-${achievementIndex}`
                );
